    ...
    <main id="main-content" className="workspace">
      <section className={`explore-panel ${mapVisible ? 'mobile-hidden' : ''}`}>
        ...
        {tab === 'nearby' && <>
          ...
          {nearby?.status === 'static' && <StaticNote/>}{nearby && <div className={`feed-note ${nearby.status === 'live' ? 'live' : ''}`}><Signal size={15}/><span>{nearby.status === 'live' ? (nearby.message || 'Partial live coverage. Only buses reported by the transit feed appear.') : nearby.status === 'static' ? 'Static stop data · routes may have changed. Live GPS unavailable.' : 'Live data is currently unavailable. Try a route or refresh.'}</span></div>}<div className="nearby-list" aria-live="polite">
