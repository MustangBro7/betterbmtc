import { arrivals, nearby, routeDetails, search, searchRoutes, UpstreamUnavailable } from "./transit";
    ...
    try {
      ...
      if (path === "/nearby") {
        ...
        return await cached(request, 30, async () => {
          try { const liveStops = await nearby(lat, lon, radius); const candidateStops = staticNearby(lat, lon, radius); try { const discovery = await nearbyVehicles(lat, lon, radius, candidateStops); return { stops: liveStops, ...discovery, status: "live", updatedAt: new Date().toISOString(), source: "BMTC unofficial public mobile API", message: "Vehicle coverage is partial: routes serving the nearest static GTFS stops are sampled, not a citywide vehicle feed." }; } catch (error) { if (!(error instanceof UpstreamUnavailable)) throw error; return { stops: liveStops, vehicles: [], coverage: { status: "unavailable", candidateRoutes: staticRouteNumbersAt(candidateStops).length, checkedRoutes: 0 }, status: "live", updatedAt: new Date().toISOString(), source: "BMTC unofficial public mobile API", message: "Nearby stops are live, but live route discovery is temporarily unavailable." }; } }
          catch (error) { if (!(error instanceof UpstreamUnavailable)) throw error; return { stops: staticNearby(lat, lon, radius), vehicles: [], status: "static", updatedAt: staticProvenance.feedPublishedAt, source: staticProvenance.source, provenance: staticProvenance, message: "Static route and stop data only. Live vehicle positions and arrival times are unavailable." }; }
        });
      }
      if (path === "/search") { const q = (url.searchParams.get("q") ?? "").trim(); if (q.length < 2 || q.length > 80) return validation("q must be 2–80 characters"); return await cached(request, 300, async () => { try { return await search(q); } catch (error) { if (error instanceof UpstreamUnavailable) return { ...staticSearch(q), status: "static", provenance: staticProvenance }; throw error; } }); }
